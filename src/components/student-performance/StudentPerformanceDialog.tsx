...
export function StudentPerformanceDialog({
  ...
  const [examDate, setExamDate] = useState(initial?.exam_date ?? todayStr());
  const [topic, setTopic] = useState(initial?.topic ?? '');
  const [totalMarks, setTotalMarks] = useState(initial?.total_marks != null ? String(initial.total_marks) : '');
  const [marksObtained, setMarksObtained] = useState(initial?.marks_obtained != null ? String(initial.marks_obtained) : '');
  ...
  function reset() {
    ...
    setExamDate(initial?.exam_date ?? todayStr());
    setTopic(initial?.topic ?? '');
    setTotalMarks(initial?.total_marks != null ? String(initial.total_marks) : '');
    setMarksObtained(initial?.marks_obtained != null ? String(initial.marks_obtained) : '');
  ...
  async function submit(e: React.FormEvent) {
    ...
    if (!Number.isFinite(total) || total <= 0) {
      setError('Enter a valid total marks value.');
      return;
    }
    if (!Number.isFinite(obtained) || obtained < 0) {
      setError('Enter a valid marks obtained value.');
      return;
    }
    if (obtained > total) {
    ...
    setError(null);
    try {
      const url = mode === 'create' ? '/api/student-performance' : `/api/student-performance/${recordId}`;
      ...
      const body =
        mode === 'create'
          ...
          : { subject_id: subjectId, exam_date: examDate, topic, total_marks: total, marks_obtained: obtained, teacher_id: teacherId };
      const res = await fetch(url, { method, headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(body) });
      const data = await res.json();
      if (!res.ok) {
        setError(data.message ?? 'Could not save this performance record.');
        return;
      }
      setOpen(false);
      router.refresh();
      ...
      {open && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 p-4">
          <div className="card w-full max-w-lg p-6">
            <h2 className="mb-4 text-lg font-semibold">{mode === 'create' ? 'Add Performance Record' : 'Edit Performance Record'}</h2>
            <form onSubmit={submit} className="space-y-4">
              ...
              <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
                ...
                <div>
                  ...
                  <input type="date" className="input" required value={examDate} onChange={(e) => setExamDate(e.target.value)} />
                </div>
              </div>
              <div>
